<?php
// このスクリプトは、Webブラウザから一度だけ実行してください。
// 実行後、セキュリティのためにこのファイルを削除してください。

// 簡単なセキュリティチェック
$secret_key = 'YOUR_SECRET_MIGRATION_KEY'; // ここをユニークな秘密鍵に変更してください
if (!isset($_GET['key']) || $_GET['key'] !== $secret_key) {
    die("Access Denied. Please provide the correct key in the URL (e.g., ?key=YOUR_SECRET_MIGRATION_KEY).");
}
...
try {
    ...
    echo "Hobby migration completed successfully.<br>";
    echo "<strong style=\"color: red;\">重要: このファイルをWebサーバーから削除してください！</strong>";
    ...
    echo "Error during hobby migration: " . htmlspecialchars($e->getMessage()) . "<br>";
    exit(1);
}
?>
